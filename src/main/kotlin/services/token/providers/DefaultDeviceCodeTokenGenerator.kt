...
class DefaultDeviceCodeTokenGenerator(
    ...
    override suspend fun generateTokens(
        ...
        val deviceCode =
            params["device_code"] ?: return Result.Failure(
                mapOf(
        ...
        if (client.clientType == "confidential" && !BCrypt.verifyer().verify(clientSecret.toCharArray(), client.clientSecret).verified) {
            return Result.Failure(
                mapOf(
        ...
        if (oauthDeviceCodeEntity != null) {
            ...
            val idToken =
                if (oauthDeviceCodeEntity.scopes.contains("openid")) {
                    jwksProvider.generateJwt(
                        subject = userId,
                        audience = clientId,
                        scopes = oauthDeviceCodeEntity.scopes,
