...
class DefaultAuthorizationCodeTokenGeneratorTest {
    ...
    @Test
    fun `generateTokens() returns generated code successfully for confidential client`() = runTest {
        ...
        val actual = authorizationCodeTokenGenerator.generateTokens(
            params = mapOf(
    ...
    @Test
    fun `generateTokens() returns generated code successfully for public client with S256 PKCE`() = runTest {
        val clientIdentifier = UUID.randomUUID()
        val clientId = UUID.randomUUID()
        val client = OAuthClientDTO(
            id = clientIdentifier,
    ...
    @Test
    fun `generateTokens() returns generated code successfully for public client with plain PKCE`() = runTest {
        val clientIdentifier = UUID.randomUUID()
        val clientId = UUID.randomUUID()
        val client = OAuthClientDTO(
            id = clientIdentifier,
    ...
    @Test
    fun `generateTokens() returns failure when code is missing`() = runTest {
        val actual = authorizationCodeTokenGenerator.generateTokens(
            params = mapOf(
                "redirect_uri" to "https://example.com/callback",
    ...
    @Test
    fun `generateTokens() returns failure when redirect_uri is missing`() = runTest {
        val actual = authorizationCodeTokenGenerator.generateTokens(
            params = mapOf(
                "code" to "some_code",
                "client_id" to "some_client_id"
    ...
    @Test
    fun `generateTokens() returns failure when authorization code is invalid`() = runTest {
        val clientIdentifier = UUID.randomUUID()
        val clientId = UUID.randomUUID()
    ...
    @Test
    fun `generateTokens() returns failure when client_id does not match authorization code client_id`() = runTest {
        val clientIdentifier = UUID.randomUUID()
    ...
    @Test
    fun `generateTokens() returns failure when redirect_uri does not match`() = runTest {
        val clientIdentifier = UUID.randomUUID()
        val clientId = UUID.randomUUID()
    ...
    @Test
    fun `generateTokens() returns failure when authorization code is already consumed`() = runTest {
        val clientIdentifier = UUID.randomUUID()
        val clientId = UUID.randomUUID()
    ...
    @Test
    fun `generateTokens() returns generated code successfully for confidential client with id_token only`() = runTest {
        val clientIdentifier = UUID.randomUUID()
        val clientId = UUID.randomUUID()
        val client = OAuthClientDTO(
            id = clientIdentifier,
    ...
    @Test
    fun `generateTokens() returns generated code successfully for confidential client with refresh_token only`() = runTest {
        val clientIdentifier = UUID.randomUUID()
        val clientId = UUID.randomUUID()
        val client = OAuthClientDTO(
            id = clientIdentifier,
    ...
    @Test
    fun `generateTokens() returns generated code successfully for confidential client with both id_token and refresh_token`() = runTest {
        val clientIdentifier = UUID.randomUUID()
        val clientId = UUID.randomUUID()
        val client = OAuthClientDTO(
            id = clientIdentifier,
    ...
    @Test
    fun `generateTokens() returns generated code successfully for public client with S256 PKCE with id_token only`() = runTest {
        val clientIdentifier = UUID.randomUUID()
        val clientId = UUID.randomUUID()
        val client = OAuthClientDTO(
            id = clientIdentifier,
    ...
    @Test
    fun `generateTokens() returns generated code successfully for public client with S256 PKCE with refresh_token only`() = runTest {
        val clientIdentifier = UUID.randomUUID()
        val clientId = UUID.randomUUID()
        val client = OAuthClientDTO(
            id = clientIdentifier,
    ...
    @Test
    fun `generateTokens() returns generated code successfully for public client with S256 PKCE with both id_token and refresh_token`() = runTest {
        val clientIdentifier = UUID.randomUUID()
        val clientId = UUID.randomUUID()
        val client = OAuthClientDTO(
            id = clientIdentifier,
    ...
    @Test
    fun `generateTokens() returns generated code successfully for public client with plain PKCE with id_token only`() = runTest {
        val clientIdentifier = UUID.randomUUID()
        val clientId = UUID.randomUUID()
        val client = OAuthClientDTO(
            id = clientIdentifier,
    ...
    @Test
    fun `generateTokens() returns generated code successfully for public client with plain PKCE with refresh_token only`() = runTest {
        val clientIdentifier = UUID.randomUUID()
        val clientId = UUID.randomUUID()
        val client = OAuthClientDTO(
            id = clientIdentifier,
    ...
    @Test
    fun `generateTokens() returns generated code successfully for public client with plain PKCE with both id_token and refresh_token`() = runTest {
        val clientIdentifier = UUID.randomUUID()
        val clientId = UUID.randomUUID()
        val client = OAuthClientDTO(
            id = clientIdentifier,
    ...
    @Test
    fun `generateTokens() returns failure when code_verifier is missing for public client with plain PKCE`() = runTest {
        val clientIdentifier = UUID.randomUUID()
        val clientId = UUID.randomUUID()
    ...
    @Test
    fun `generateTokens() returns failure when code_verifier is missing for public client with S256 PKCE`() = runTest {
        val clientIdentifier = UUID.randomUUID()
        val clientId = UUID.randomUUID()
    ...
    @Test
    fun `generateTokens() returns failure when code_verifier does not match code_challenge for plain PKCE`() = runTest {
        val clientIdentifier = UUID.randomUUID()
        val clientId = UUID.randomUUID()
    ...
    @Test
    fun `generateTokens() returns failure when code_verifier does not match code_challenge for S256 PKCE`() = runTest {
        val clientIdentifier = UUID.randomUUID()
        val clientId = UUID.randomUUID()
    ...
    @Test
    fun `generateTokens() returns failure when code_challenge is missing for public client with plain PKCE during token request`() = runTest {
        val clientIdentifier = UUID.randomUUID()
        val clientId = UUID.randomUUID()
    ...
    @Test
    fun `generateTokens() returns failure when code_challenge is missing for public client with S256 PKCE during token request`() = runTest {
        val clientIdentifier = UUID.randomUUID()
        val clientId = UUID.randomUUID()
